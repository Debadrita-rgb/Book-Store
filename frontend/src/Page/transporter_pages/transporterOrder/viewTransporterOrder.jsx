...
const ViewTransporterOrder = () => {
  ...
  const getOrders = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/transporter/get-assigned-order`, {
        headers: {
  ...
  const filteredItems = (orders || [])
    ...
    .map((item, index) => ({
      ...
      _id: item._id,
      viewPath: `/transporter/view-transporter-order-details/${item._id}`,
    }));
  ...
  const handleChangeStatus = async (id, status) => {
    try {
      ...
      const response = await axios.put(
        `${BASE_URL}/transporter/update-order-status/${id}`,
        { orderStatus: status },
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export default ViewTransporterOrder;
